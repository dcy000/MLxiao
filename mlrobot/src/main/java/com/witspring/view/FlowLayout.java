...
public class FlowLayout extends ViewGroup {
    ...
    private int numLines, trueHeight, maxLines, trueChildCount;
    ...
    public void setLines(int lines) {
        if (lines < getNumLines()) {
            int newHeight = getTrueHeight() * lines / getNumLines();
    ...
    public List<List<View>> getLineViews() {
        return mLines;
    }
    ...
    public FlowLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.WSFlowLayout, defStyle, 0);

        try {
            int index = a.getInt(R.styleable.WSFlowLayout_android_gravity, -1);
            if(index > 0) {
                setGravity(index);
            }
        } finally {
            ...
            if(lineWidth + childWidth > sizeWidth) {
                ...
                if (maxLines != 0) {
                    ...
                    if (numLines > maxLines) {
                        numLines = maxLines;
                        trueChildCount = i;
                        int moreChildCount = childCount - trueChildCount;
                        if (moreChildCount > 0) {
                            for (int j = 0; j < moreChildCount; j++) {
                                removeViewAt(i);
                            }
                        }
                        break;
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            TypedArray a = c.obtainStyledAttributes(attrs, R.styleable.WSFlowLayout_Layout);

            try {
                gravity = a.getInt(R.styleable.WSFlowLayout_Layout_android_layout_gravity, -1);
            } finally {
                a.recycle();
            }
        }
