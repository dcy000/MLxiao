    ...
    private void dealData() {
        if (data!=null){
            ...
            if (!TextUtils.isEmpty(target)&&target.length()>1){
                stringSpeak="根据您的评估结果，您有可能是原发性高血压，并有可能存在对"+target.substring(0,target.length()-1)+"等靶器官的损害。";
            }else{
                stringSpeak="根据您的评估结果，您有可能是原发性高血压。";
            }
            ...
            stringSpeak+="建议您健康饮食，合理运动，根据康复疗程进行生活干预，必要时请咨询健康顾问";
            speak(stringSpeak);
