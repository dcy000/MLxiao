...
/**
 * Created by wecent on 2018/8/27.
 */

public class SelecterView extends View {

    private Context context;
    private List<String> strings = new ArrayList<String>();//数据集合
    private int visibleSize = 1;//可见值个数
    ...
    private SelecterView.OnValueChangeListener mListener;  //滑动后数值回调
    private int n;
    private float downX;
    ...
    public SelecterView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SelecterView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public void setOnValueChangeListener(SelecterView.OnValueChangeListener listener) {
        mListener = listener;
    }

    private void notifyValueChange() {
        if (null != mListener) {
            mListener.onValueChange(n);
        }
    }
    ...
    private void initAttrs(AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs,
                R.styleable.SelecterView);
        //两种字体颜色和字体大小
        visibleSize = typedArray.getInteger(R.styleable.SelecterView_selecter_VisibleSize, 5);
        selectedTextSize = typedArray.getFloat(R.styleable.SelecterView_selecter_SelectedTextSize, 50);
        selectedColor = typedArray.getColor(R.styleable.SelecterView_selecter_SelectedTextColor, context.getResources().getColor(android.R.color.black));
        textSize = typedArray.getFloat(R.styleable.SelecterView_selecter_TextSize, 40);
        textColor = typedArray.getColor(R.styleable.SelecterView_selecter_TextColor, context.getResources().getColor(android.R.color.darker_gray));
    }
     ...
     * @param sizes 可见数
     */
    public void setVisibleSize(int sizes) {
        if (visibleSize > 0) {
            visibleSize = sizes;
            invalidate();
        }
