...
public class CallManager {
    ...
    private int mSoundId;  // 可用于加载或卸载声音
    ...
    private void loadSound() {
        if (isInComingCall) {
            mSoundId = soundPool.load(context, R.raw.sound_call_incoming, 1);
        } else {
            mSoundId = soundPool.load(context, R.raw.sound_calling, 1);
        }
    }
    ...
    private void playCallSound() {
        ...
        if (soundPool != null) {
            streamID = soundPool.play(
                    mSoundId, // 播放资源id；就是加载到SoundPool里的音频资源顺序
                    0.5f,   // 左声道音量
                    0.5f,   // 右声道音量
                    1,      // 优先级，数值越高，优先级越大
