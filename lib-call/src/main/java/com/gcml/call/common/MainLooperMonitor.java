...
public class MainLooperMonitor {
    ...
    private void dumpCrashToSdcard(String info) throws IOException {
        if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            Timber.tag(TAG).w("Sdcard unmounted, skip dump crash");
            return;
        }
        ...
        try {
            ...
            writer.println(info);
            writer.close();
            Timber.tag(TAG).i("Dump crash to sdcard success");
        } catch (Exception e) {
            Timber.tag(TAG).w(e, "Dump crash to sdcard failed");
        }
    }
    ...
    private void dumpCrashToSdcard(Throwable throwable) throws IOException {
        ...
        try {
            ...
            Timber.tag(TAG).w("Dump crash to sdcard failed");
        }
    }
