...
public class ImageLoader implements IImageLoader {
    ...
    private ImageLoader() {
        ServiceLoader<IImageLoader> loaders = ServiceLoader.load(IImageLoader.class);
        for (IImageLoader imageLoader : loaders) {
            if (imageLoader != null) {
                this.loaders.put(imageLoader.id(), imageLoader);
            }
        }
    }
