...
public class TvChannelPresenterImp implements ITvChannelInteractor.OnParseExcelListener, ITvChannelPresenter {
    ...
    private MLRecognizerListener recognizerListener;
    ...
    private void speakAndListen() {
        synthesizerListener = new SynthesizerListener() {
            ...
            public void onCompleted(SpeechError speechError) {
                ...
                Runnable command = new Runnable() {
                    ...
                    public void run() {
                        if (!isOnPause) {
                            MLVoiceRecognize.startRecognize((TvChannelActivity) tvChannelActivity, recognizerListener);
                        }
                    }
                };
                if (threadPool!=null) {
                    threadPool.scheduleWithFixedDelay(command, 2000,2000, java.util.concurrent.TimeUnit.MILLISECONDS);
                }
            }
        ...
        recognizerListener = new MLRecognizerListener() {
            ...
            public void onMLResult(String result) {
                ...
                if (inSpell.matches("(.*)fanhui(.*)")) {
                    tvChannelActivity.closeActivity();
                    return;
                }
        ...
        MLVoiceSynthetize.startSynthesize((TvChannelActivity) tvChannelActivity, "主人，您想看哪个电视节目？", synthesizerListener, false);
