 ...
 * <p> 1. 打开相机
 * <ul>
 * <li>{@link Camera#reconnect() }</li>
 * <li>{@link Camera#open(int cameraId) }</li>
 * </ul>
 * <p>
 * <p> 2. 预览相机
 ...
 * <p> 3. 设置预览回调
 * <ul>
 * <li>{@link Camera#setPreviewCallback}</li>
 * <li>{@link Camera#setOneShotPreviewCallback}</li>
 * <li>{@link Camera#setPreviewCallbackWithBuffer}, {@link Camera#addCallbackBuffer(byte[])}</li>
 * </ul>
 * <p> 4. 预览回调时数据处理
 * <p>{@link Camera.PreviewCallback#onPreviewFrame(byte[], Camera)}
 * <p>
 * <p>注意：
 * <p>
 * <p> 1. 设置预览回调时，
 * {@link Camera#setPreviewCallback}
 * 和 {@link Camera#setOneShotPreviewCallback} 都存在内存抖动.
 * <p>
 ...
 * 不要同步执行过于复杂的逻辑操作，会阻塞 Camera，导致帧率下降。</p>
 * <p>
 * <p> 4. 处理好关闭 Camera {@link Camera#release()}
 */
public class PreviewHelper
    ...
    public void onCreate(android.arch.lifecycle.LifecycleOwner owner) {

    }

    @OnLifecycleEvent(android.arch.lifecycle.Lifecycle.Event.ON_START)
    public void onStart(android.arch.lifecycle.LifecycleOwner owner) {
    ...
    public void onResume(android.arch.lifecycle.LifecycleOwner owner) {

    }

    @OnLifecycleEvent(android.arch.lifecycle.Lifecycle.Event.ON_PAUSE)
    public void onPause(android.arch.lifecycle.LifecycleOwner owner) {

    }

    @OnLifecycleEvent(android.arch.lifecycle.Lifecycle.Event.ON_STOP)
    public void onStop(android.arch.lifecycle.LifecycleOwner owner) {
    ...
    public void onDestroy(android.arch.lifecycle.LifecycleOwner owner) {
    ...
    @OnLifecycleEvent(android.arch.lifecycle.Lifecycle.Event.ON_ANY)
    public void onLifecycleChanged(android.arch.lifecycle.LifecycleOwner owner, android.arch.lifecycle.Lifecycle.Event event) {
    ...
    private void openAndPreviewCamera(SurfaceHolder holder) {
        cameraHandler().post(new Runnable() {
            @Override
            public void run() {
                if ((mCamera = CameraUtils.reconnect(mCamera)) != null
                        || (mCamera = CameraUtils.openByFacing(mCameraId)) != null) {
                    configCameraInternal();
