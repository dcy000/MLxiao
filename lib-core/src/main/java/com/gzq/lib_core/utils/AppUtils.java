...
public class AppUtils {
    ...
    public static void installApp(String filePath) {
        App.getApp().startActivity(getInstallAppIntent(filePath));
    }
    ...
    public static void installApp(Activity activity, String filePath, int requestCode) {
        activity.startActivityForResult(getInstallAppIntent(filePath), requestCode);
    }
    ...
    public static void uninstallApp(String packageName) {
        if (DataUtils.isNullString(packageName)) {
            return;
        }
        App.getApp().startActivity(getUninstallAppIntent(packageName));
    }
    ...
    public static boolean isAppForeground() {
        ActivityManager manager = (ActivityManager) App.getApp().getSystemService(Context.ACTIVITY_SERVICE);
        ...
        for (ActivityManager.RunningAppProcessInfo info : infos) {
            if (info.importance == ActivityManager.RunningAppProcessInfo.IMPORTANCE_FOREGROUND) {
                return info.processName.equals(App.getApp().getPackageName());
            }
        }
        return false;
    }
    ...
    public static boolean isAppBackground() {
        ...
        if (!tasks.isEmpty()) {
            ...
            return !topActivity.getPackageName().equals(App.getApp().getPackageName());
        }
        return false;
    }
    ...
    public static boolean cleanAppData(File... dirs) {
        boolean isSuccess = FileUtils.cleanInternalCache();
        isSuccess &= FileUtils.cleanInternalDbs();
        isSuccess &= FileUtils.cleanInternalSP();
        isSuccess &= FileUtils.cleanInternalFiles();
        isSuccess &= FileUtils.cleanExternalCache();
        for (File dir : dirs) {
    ...
    private static Intent getInstallAppIntent(String filePath) {
        ...
        Uri contentUri = FileUtils.getUriForFile(apkfile);
