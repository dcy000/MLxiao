...
import com.gcml.lib_ecg.ecg.BackGround;
import com.gcml.lib_ecg.ecg.DrawThreadPC80B;
import com.gcml.lib_ecg.ecg.ECGBluetooth;
import com.gcml.lib_ecg.ecg.ReceiveService;
import com.gcml.lib_ecg.ecg.StaticReceive;
                ...
                case StaticReceive.MSG_DATA_ECG_STATUS_CH: {
                    switch (msg.arg1) {
                        case StatusMsg.FILE_TRANSMIT_START: {
                            // 接收文件 receive file
                            setMSG(Box.getApp().getResources().getString(R.string.measure_ecg_file_ing));

                        }
                        break;
                        case StatusMsg.FILE_TRANSMIT_SUCCESS: {
                            setMSG(Box.getApp().getResources().getString(R.string.measure_ecg_file_end));
                        }
                        break;
                        case StatusMsg.FILE_TRANSMIT_ERROR: {
                            setMSG(Box.getApp().getResources().getString(R.string.measure_ecg_time_err));
                        }
                        break;
                        case StaticReceive.MSG_DATA_TIMEOUT: {
                            setMSG(Box.getApp().getResources().getString(R.string.measure_ecg_time_out));
                        }
                        break;
                        case 4: {
                            // 准备阶段波形   ready wave
                            ...
                            if (data.getBoolean("bLeadoff")) {
                                setMSG(Box.getApp().getResources().getString(R.string.measure_lead_off));
                            } else {
                                setMSG(" ");
                            }
                        }
    ...
    private void setMSG(String msg) {
        if (mMainPc80BMSG != null) {
            ...
            if (msg != null && !msg.equals("")) {
                if (msg.equals("0")) {
                    mMainPc80BMSG.setText(Box.getApp().getResources().getString(
