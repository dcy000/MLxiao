...
import com.example.module_mall.adapter.OrderAdapter;
...
public class OrderListActivity extends ToolbarBaseActivity implements View.OnClickListener {
    ...
    public void initParams(Intent intentArgument) {
        ...
        Box.getRetrofit(MallAPI.class)
                ...
                .compose(RxUtils.<List<Orders>>httpResponseTransformer())
                .as(RxUtils.<List<Orders>>autoDisposeConverter(this))
                    ...
                    if ((countItem - 1) == maxPosition && isSlidingUp) {
                        ...
                        if (mlist.size() >= 1300) {
                            ...
                            Box.getRetrofit(MallAPI.class)
                                    ...
                                    .compose(RxUtils.<List<Orders>>httpResponseTransformer())
                                    .as(RxUtils.<List<Orders>>autoDisposeConverter(OrderListActivity.this))
                                    .subscribe(new CommonObserver<List<Orders>>() {
                                        @Override
                                        public void onNext(List<Orders> orders) {
