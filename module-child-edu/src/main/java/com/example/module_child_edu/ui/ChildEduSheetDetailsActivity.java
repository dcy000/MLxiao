...
import com.example.module_child_edu.AutoLoadMoreHelper;
...
public class ChildEduSheetDetailsActivity extends ToolbarBaseActivity {
    ...
    public int layoutId(Bundle savedInstanceState) {
        return R.layout.ce_activity_sheet_details;
    }
    ...
    public void initParams(Intent intentArgument) {
        sheetModel = intentArgument.getParcelableExtra("sheet");
    ...
    private void loadSheet(int page, int limit) {
        Box.getRetrofit(ChildAPI.class)
                .getChildEduSongListBySheetId(page, limit,
                        sheetModel.getId(), 3, "")
                .compose(RxUtils.<List<SongModel>>httpResponseTransformer())
                .doOnTerminate(new Action() {
                    @Override
                    public void run() throws Exception {
                        hideLoadingDialog();
                ...
                .as(RxUtils.<List<SongModel>>autoDisposeConverter(this))
