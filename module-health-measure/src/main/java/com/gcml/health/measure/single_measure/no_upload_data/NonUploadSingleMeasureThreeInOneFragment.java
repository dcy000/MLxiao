    ...
    protected void onMeasureFinished(String... results) {
        ...
        if (results.length == 2) {
            if (results[0].equals("bloodsugar")) {
                MLVoiceSynthetize.startSynthesize(UM.getApp(), UM.getString(R.string.this_time_blood_glucose) + results[1]);
                if (measureItemChanged!=null){
                    measureItemChanged.onChanged(2);
                }
            }
            if (results[0].equals("cholesterol")) {
                MLVoiceSynthetize.startSynthesize(UM.getApp(), UM.getString(R.string.this_time_cholesterol)+ results[1]);
                if (measureItemChanged!=null){
            ...
            if (results[0].equals("bua")) {
                MLVoiceSynthetize.startSynthesize(UM.getApp(), UM.getString(R.string.this_time_uric_acid) + results[1]);
                if (measureItemChanged!=null){
