    ...
    protected void onMeasureFinished(String... results) {
        ...
        if (results.length == 2) {
            if (results[0].equals("bloodsugar")) {
                MLVoiceSynthetize.startSynthesize(UtilsManager.getApplication(), "您好，您本次测量血糖" + results[1]);
                if (measureItemChanged!=null){
                    measureItemChanged.onChanged(2);
                }
            }
            if (results[0].equals("cholesterol")) {
                MLVoiceSynthetize.startSynthesize(UtilsManager.getApplication(), "您好，您本次测量胆固醇" + results[1]);
                if (measureItemChanged!=null){
            ...
            if (results[0].equals("bua")) {
                MLVoiceSynthetize.startSynthesize(UtilsManager.getApplication(), "您好，您本次测量尿酸" + results[1]);
                if (measureItemChanged!=null){
