...
public class ChildEduEntertainmentActivity extends ToolbarBaseActivity {
...
//        Handlers.bg().post(fetchJokesRunnable);
    ...
    private Runnable fetchJokesRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if (results == null) {
                MLVoiceSynthetize.startSynthesize(UM.getApp(),"没有笑话了， 让我再想想");
                return;
            }
            ...
            if (size == 1) {
                MLVoiceSynthetize.startSynthesize(UM.getApp(),models.get(0).getContent());
                return;
            }
            if (random == null) {
                random = new Random();
            ...
            MLVoiceSynthetize.startSynthesize(UM.getApp(),models.get(i % size).getContent());
        }
    };
