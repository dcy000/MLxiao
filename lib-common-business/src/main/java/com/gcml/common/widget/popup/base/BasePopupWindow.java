package com.gcml.common.widget.popup.base;
...
public abstract class BasePopupWindow implements BasePopup {
    ...
    private void initView(Activity context, int w, int h) {
        ...
        // 默认占满全屏
        mPopupWindow = new NewPopupWindow(mPopupView, w, h);
        // 指定透明背景，back键相关
    ...
    private void tryToShowPopup(int res, View v) throws Exception {
        ...
        // 什么都没传递，取顶级view的id
        if (res == 0 && v == null) {
            int y=mContext.getResources().getDimensionPixelOffset(R.dimen.base_popup_hight);
            mPopupWindow.showAtLocation(mContext.findViewById(android.R.id.content), Gravity.TOP, 0, y);
        }
