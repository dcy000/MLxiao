...
import com.gcml.common.repository.utils.ZipHelper;
...
public class HttpLogInterceptor implements Interceptor {
    ...
    public HttpLogInterceptor(
            @Nullable ClientModule.HttpInterceptor interceptor,
    ...
    public Response intercept(@NonNull Chain chain) throws IOException {
        ...
        if (mHttpInterceptor != null) {
            return mHttpInterceptor.onHttpResponse(bodyString, chain, originalResponse);
        }
            ...
            if (logResponse) {
                Timber.w("HTTP RESPONSE >>>%n「 %s 」%nResponse Content:%n%s",
                        ...
                        isJson(responseBody.contentType()) ?
                                CharacterHelper.jsonFormat(bodyString) : isXml(responseBody.contentType()) ?
                                CharacterHelper.xmlFormat(bodyString) : bodyString);
            }
