...
public class WheelPicker extends View {
    ...
    /** 选中时候的字体颜色 */
    private int selectedColor = Color.parseColor("#1dc549");//
    /** 蒙板高度 */
    ...
    private synchronized void goonMove(final int move) {
        new Thread(new Runnable() {
            ...
            public void run() {
                // TODO Auto-generated method stub
                int distence = 0;
                while (distence < unitHeight * moveNumber) {
                    try {
                        Thread.sleep(5);
    ...
    private void init(Context context, AttributeSet attrs) {
        TypedArray attribute = context.obtainStyledAttributes(attrs,
                R.styleable.WSWheelPicker);
        unitHeight = (int) attribute.getDimension(
                R.styleable.WSWheelPicker_ws_unitHight, 32);
        normalFont = attribute.getDimension(
                R.styleable.WSWheelPicker_ws_normalTextSize, 18.0f);
        selectedFont = attribute.getDimension(
                R.styleable.WSWheelPicker_ws_selecredTextSize, 22.0f);
        itemNumber = attribute.getInt(R.styleable.WSWheelPicker_ws_itemNumber, 7);
        normalColor = attribute.getColor(
                R.styleable.WSWheelPicker_ws_normalTextColor, Color.parseColor("#999999"));
        selectedColor = attribute.getColor(
                R.styleable.WSWheelPicker_ws_selecredTextColor, Color.parseColor("#1dc549"));
        lineColor = attribute.getColor(R.styleable.WSWheelPicker_ws_lineColor, Color.parseColor("#DADADA"));
        maskHight = attribute.getDimension(
                R.styleable.WSWheelPicker_ws_maskHight, 48.0f);
        noEmpty = attribute.getBoolean(R.styleable.WSWheelPicker_ws_noEmpty,
                false);
        isEnable = attribute.getBoolean(R.styleable.WSWheelPicker_ws_isEnable,
                true);
        attribute.recycle();
        ...
        public void drawSelf(Canvas canvas) {
            ...
            if (isSelected()) {
                ...
                moveToSelect = moveToSelect > 0 ? moveToSelect : moveToSelect
                        * (-1);
                textPaint.setTextSize(selectedFont);//DensityUtil.sp2px(ctx ,selectedFont));
            } else {
                textPaint.setColor(normalColor);
                textPaint.setTextSize(normalFont);//DensityUtil.sp2px(ctx ,normalFont));
            }
